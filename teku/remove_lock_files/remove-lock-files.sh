...
if [ -z "$referrentLockFile" ]; then
    echo "missing referrentLockFile parameter: $0 | Sample: '/var/lib/teku-validator/validator_keys/keystore-m_12381_3600_0_0_0-1710000000000.lock'"
    exit 1
fi
...
# Iterate over files in the folder and delete any .lock files
for f in $files; do
    if [[ $f == *.lock ]]; then
        echo "Deleting $f"
        rm "$directory_path/$f"
