 ...
 *     --snapshot.statuses=active_ongoing \
 *     --snapshot.includeFields=index,balance,status \
...
class RewardsCalculator {
    ...
    async beaconGet(path, params = undefined, timeoutMs = 0) {
        ...
        const url = usp.toString() ? `${base}${path}?${usp.toString()}` : `${base}${path}`;
    ...
    static indexByIndexField(rows) {
        const m = new Map();
        for (const r of rows) m.set(Number(r.index), r);
        return m;
    }

    static computeDailyFromSnapshots(prevRows, currRows) {
        const prev = RewardsCalculator.indexByIndexField(prevRows);
        const curr = RewardsCalculator.indexByIndexField(currRows);
        let clDeltaWei = 0n;
        let withdrawalsWei = 0n;
        let elIncomeWei = 0n;
        for (const [idx, rNow] of curr.entries()) {
            const nowGwei = BigInt(rNow.balance ?? rNow.balance_gwei ?? 0);
            const wasGwei = BigInt((prev.get(idx)?.balance ?? prev.get(idx)?.balance_gwei ?? 0));
            clDeltaWei += (nowGwei - wasGwei) * 1_000_000_000n;
            if (rNow.withdrawals_wei)  withdrawalsWei += BigInt(rNow.withdrawals_wei);
            if (rNow.el_income_wei)    elIncomeWei    += BigInt(rNow.el_income_wei);
        }
        const totalWei = clDeltaWei + withdrawalsWei + elIncomeWei;
        return { clDeltaWei, withdrawalsWei, elIncomeWei, totalWei };
    }
    ...
    // ---------- Subcommand: calc-from-snapshots ----------
    static async runCalcFromSnapshotsCLI() {
        const cfg = { calcFromSnapshots: {
            snapDir: "/var/data",
            date: null,          // YYYY-MM-DD
            indices: null,       // CSV or array
            verbose: true
        }};
        loadFromArgumentsUtil(cfg);
        const s = cfg.calcFromSnapshots;
        if (!s.date) throw new Error("Please provide --calcFromSnapshots.date=YYYY-MM-DD");
        const d = s.date;
        const prev = new Date(d + "T00:00:00Z"); prev.setUTCDate(prev.getUTCDate() - 1);
        const dPrev = prev.toISOString().slice(0,10);
        const fPrev = `${s.snapDir}/snap_${dPrev}.jsonl`;
        const fCurr = `${s.snapDir}/snap_${d}.jsonl`;
        console.log("runCalcFromSnapshotsCLI", cfg, prev, dPrev, fPrev, fCurr);
        const prevRows = RewardsCalculator.readSnapshotFile(fPrev);
        const currRows = RewardsCalculator.readSnapshotFile(fCurr);
        const idxList = RewardsCalculator.toArrayMaybeCSV(s.indices);
        const filter = (rows) => !idxList ? rows : rows.filter(r => idxList.includes(String(r.index)) || idxList.includes(Number(r.index)));
        const res = RewardsCalculator.computeDailyFromSnapshots(filter(prevRows), filter(currRows));
        const out = {
            date: d,
            cl_delta_wei: res.clDeltaWei.toString(),
            withdrawals_wei: res.withdrawalsWei.toString(),
            el_income_wei: res.elIncomeWei.toString(),
            total_wei: res.totalWei.toString(),
            total_eth: (Number(res.totalWei) / 1e18).toFixed(12),
        };
...
(async () => {
    ...
    if (argv[0] === "snapshot") {
        try {
            await RewardsCalculator.runSnapshotCLI();
            process.exit(0);
        } catch (err) {
        ...
        try {
            await RewardsCalculator.runCalcFromSnapshotsCLI();
            process.exit(0);
        } catch (err) {
            console.error(err?.message || err);
            process.exit(1);
